...
public class RootUserServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		// Get the root user command from the .html/.jsp front-end
		String sqlStatement = request.getParameter("command"); // inbound user command
		String btnPress = request.getParameter("btnPress");
		// Set up the message string for the response back to the root front-end
			...
			try {
				...
				if (connection == null) {
					...
					if (inboundCommand.toLowerCase().startsWith("select")) {
						...
						// business logic for root user
						if (inboundCommand.toLowerCase().contains("shipments")) {
							// Business logic detected
							String businessLogicCommand = "update suppliers set status = status + 5 where snum in (select distinct snum from shipments where quantity >= 100)";
							...
							message = "<table><tr style='background-color: #00ff00;'><td><font color='#000000'>The statement executed successfully. "
									+ mysqlUpdateValue
									+ " row(s) affected. <br><br> Business Logic Detected! - Updating Supplier Status. <br><br> Business Logic updated "
									+ businessLogicUpdateValue + " supplier status marks.</font></td></tr></table>";
						} else {
							// regular update
							message = "<table><tr style='background-color: #00ff00;'><td><font color='#000000'>The statement executed successfully. "
	...
	private Connection getDBConnection() {
		...
		try (InputStream input = getServletContext().getResourceAsStream("/WEB-INF/lib/root.properties")) {
